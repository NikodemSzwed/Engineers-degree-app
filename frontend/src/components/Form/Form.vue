<template>
    <Form
        ...
        fluid
    >
        <div class="flex flex-col gap-1" v-for="(field, i) in props.fields">
            <FormField
                v-if="field.component !== 'customNoFormField'"
                v-slot="$field"
                :name="field.name"
            ...
            <div v-else>
                <slot :name="'input-' + field.name" v-bind="{ field }"></slot>
            </div>
        </div>
...
<script setup>
    ...
    const globalResolver = ({ values }) => {
        ...
        for (const field of props.fields) {
            ...
            if (
                !maxLengthDefined &&
                (field.component == 'InputText' || field.component == 'Password') &&
                values[field.name] &&
                values[field.name].length > 255
            ) {
