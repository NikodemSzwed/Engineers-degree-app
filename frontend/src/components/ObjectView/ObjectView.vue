<template>
    <div class="flex flex-col gap-3">
        <Card>
            <template #content>
                <DataTable
                    :items="localItemSimpleFields"
                    :columns="simpleColumns"
                    :showInteractions="false"
                    :showAdvancedObjectView="false"
                    :show-header="false"
                    :showPaginator="false"
                ></DataTable>
            </template>
        ...
        <Card v-for="item of localItemComplexFields">
            ...
            <template #content>
                <DataTable
                    :items="item.value"
                    :columns="props.complexFieldsColumns[item.key]"
                    :showInteractions="false"
                    :showAdvancedObjectView="false"
                ></DataTable>
            </template>
...
<script setup>
    ...
    const simpleColumns = ref([
        { label: 'Nazwa właściwości', field: 'key', dataKay: true },
        { label: 'Wartość', field: 'value' },
    ]);
    ...
    onMounted(() => {
        ...
        localItemSimpleFields.value = Object.entries(localItem.value)
            .filter(
                ([key, value]) =>
                    typeof value !== 'object' && !key.includes('_json') && (props.fieldMap[key]?.show ?? true)
            )
            .map(([key, value]) => {
                let val = value;
                if (props.fieldMap[key]?.date)
                    val = formatDate(value, props.fieldMap[key]?.format || 'dd.MM.yyyy HH:mm');
