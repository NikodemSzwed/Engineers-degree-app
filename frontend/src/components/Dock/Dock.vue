<template>
    <Dialog v-model:visible="localVisible" :header="header" :position="position" :modal="false" :class="{
        'max-h-[50rem] w-[20rem] mr-0': direction === 'vertical',
        'max-w-[60rem] mb-0': direction === 'horizontal',
    }" :pt="{
        content: {
        ...
        <div class="flex gap-3" :class="{
            ...
            'w-fit flex-row justify-center': direction === 'horizontal',
        }">
            <slot name="content" />
        </div>
    </Dialog>
