<template>
    ...
    <DataTable
        ...
        :paginator="!virtualScrollerActive && props.showPaginator"
        :paginatorTemplate="{
        ...
        :filterDisplay="advancedFilters ? 'menu' : 'none'"
    ...
    >
        ...
        <template v-if="props.showHeader" #header>
            <div class="flex flex-col justify-between gap-2 pb-1 lg:flex-row lg:gap-3 lg:pb-2">
                <div class="flex w-full flex-row justify-between gap-2 lg:justify-start lg:gap-3">
                ...
                >
                    ...
                    <Popover
                        ref="advancedMenuPopover"
                        v-if="advancedInteractionsAvailable"
                        class="max-w-sm lg:min-w-sm"
                    >
                        <div class="flex flex-col gap-3">
                            <div class="flex w-full flex-row justify-between gap-3" v-if="advancedFiltersAvailable">
                                <span>Zaawansowane filtry </span>
                                <ToggleSwitch v-model="advancedFilters" @change="advancedFiltersToggle" />
                            </div>
                            <div class="flex w-full flex-row justify-between gap-3" v-if="advancedColumnsAvailable">
        ...
        <Column
            ...
            v-show="col.show == false ? false : true"
            :frozen="frozenColumns.includes(col)"
            :dataType="col.type == 'list' ? 'any' : col.type"
            :showFilterMatchModes="col.type == 'list' ? false : true"
            :showFilterOperator="col.type == 'list' ? false : true"
            :showAddButton="col.type == 'list' ? false : true"
            v-bind="col.colProps"
            showClearButton
            class="w-fit min-w-35"
        >
            <template #body="{ data }">
        ...
        <Column v-if="props.showAdvancedObjectView" class="w-4 !text-end">
            <template #body="{ data }">
                <Button
                    icon="pi pi-search"
                    @click="$emit('showAdvancedObjectView', data)"
                    severity="secondary"
                    rounded
                ></Button>
            </template>
...
<script setup>
    ...
    for (const col of props.columns) {
        ...
        if (col.overrideFilter) defaultfilters.value[col.field] = col.overrideFilter;
            ...
            defaultfilters.value[col.field] = {
