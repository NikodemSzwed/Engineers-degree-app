<template>
    ...
    <div class="flex h-screen w-full overflow-hidden">
        <div class="bg-emphasis relative flex max-w-screen flex-1 flex-col overflow-y-auto">
            <div class="flex w-full flex-row items-center justify-between" v-if="validated">
                <div
                    class="mx-1 mt-1 rounded-xl bg-[var(--p-card-background)] p-2 text-lg shadow lg:mx-3 lg:mt-3 lg:p-3"
                >
                    Monitor: {{ displayName }}
                </div>
                <Button class="mx-1 mt-1 lg:mx-3 lg:mt-3" raised @click="showEmptyDock()">Zgłoś problem</Button>
            </div>
            <div class="mx-1 mt-1 flex flex-1 flex-col items-center lg:mx-3 lg:mt-3" v-if="validated">
                <div class="flex w-full flex-1 flex-row flex-wrap items-stretch justify-center gap-1 lg:gap-3">
                    <Card
                        class="min-w-[55vw] flex-1 overflow-hidden lg:min-w-[35vw]"
                        :class="{
                            'outline-primary rounded-xl outline-2': item.selected,
                    ...
                    >
                        <template #title>
                            <div class="flex w-full flex-row justify-between">
                                ...
                                <div class="flex flex-row">
                                    <div
                                        v-if="item.data.alerts.length > 0"
                                        :class="{
                                    ...
                                    >
                                        <span class="mr-1">{{
                                            item.data.alerts.length > 3 ? '3+' : item.data.alerts.length
                                        }}</span>
                                        <i
                                    ...
                                    <div
                                        class="flex w-10 cursor-pointer items-center justify-end"
                        ...
                        <template #content>
                            <div class="h-full overflow-y-auto">
                                <div v-if="item.ETID == 1" class="flex h-full min-h-[75vh] w-full">
                                    <MapInterface
                                        ref="mapInterface"
                                        class="flex-1"
                                        :data="item.data.mapData"
                                        @click.stop=""
                                        @selected="selectObject"
                                        @deselected="deselect(item)"
                                    >
                                    </MapInterface>
                                </div>
                                <div v-else-if="item.ETID == 2">
                                    <div class="flex flex-col gap-1">
                                        <div
                                            class="flex w-full flex-row gap-1 rounded-xl lg:gap-3"
                                            v-for="(field, j) in orderFields"
                                            :class="{ 'bg-surface-200': j % 2 === 1, 'bg-surface-100': j % 2 === 0 }"
                                        >
                                            <div class="min-w-40 p-2 lg:p-3">{{ field.label }}</div>
                                            <div v-if="field.key == 'State'" class="p-2 lg:p-3">
                                                <Tag
                                                    :severity="getSeverity(item.data[field.key])"
                                                    :value="getMessage(item.data[field.key])"
                                                ></Tag>
                                            </div>
                                        ...
                                        <div class="bg-surface-100 flex flex-col rounded-xl">
                                            <div class="min-w-40 p-2 lg:p-3">Dostawy:</div>
                                            <div class="mx-2 mb-2 flex flex-col gap-1 lg:mx-3 lg:mb-3">
                                                <div
                                                    class="rounded-xl bg-white p-1 pl-2 lg:p-3 lg:pl-3"
                                ...
                                <div v-else-if="item.ETID == 3">
                                    <div class="m-1 flex flex-col gap-1">
                                        <div
                                            class="flex min-w-40 flex-1 flex-row justify-between gap-1 rounded-xl p-2 lg:gap-3 lg:p-3"
                                            v-for="(order, j) in item.orders"
                                            :class="{
                                                'bg-surface-200': j % 2 === 1,
                                                'bg-surface-100': j % 2 === 0,
                                                'outline-primary rounded-xl outline-2': order.selected,
                                            }"
                                            @click.stop="selectObject(order)"
                                        >
                                            <div>{{ order.name }}</div>
                                            <div
                                                v-if="order.alerts.length > 0"
                                                :class="{
                                            ...
                                            >
                                                <span class="mr-1">{{
                                                    order.alerts.length > 3 ? '3+' : order.alerts.length
                                                }}</span>
                                                <i
                                                    class="pi pi-exclamation-triangle"
                                                    :class="{
                ...
                <Dock v-model:visible="dockVisible" header="Zgłoś problem">
                    <template #content>
                        <div
                            class="bg-emphasis flex h-30 w-full cursor-pointer items-center justify-center rounded-xl shadow"
            ...
            <div v-if="!validated" class="flex flex-1 items-center justify-center">
                <Card>
                    <template #content>
    ...
    <Dialog v-model:visible="itemDialog" modal maximizable class="w-[70%]">
        ...
        <template #default>
            <div class="h-full overflow-y-auto">
                <div v-if="showItem.ETID == 1" class="flex h-full min-h-[75vh] w-full">
                    <MapInterface
                        ref="mapInterfaceDialog"
                        class="flex-1"
                        :data="showItem.data.mapData"
                ...
                <div v-else-if="showItem.ETID == 2">
                    <div class="flex flex-col gap-1">
                        ...
                        >
                            ...
                            <div v-if="field.key == 'State'" class="p-2 lg:p-3">
                                <Tag
                                    :severity="getSeverity(showItem.data[field.key])"
                                    :value="getMessage(showItem.data[field.key])"
                                ></Tag>
                            </div>
                            <div v-else-if="field.key == 'deadline'" class="p-2 lg:p-3">
                ...
                <div v-else-if="showItem.ETID == 3">
                    <div class="m-1 flex flex-col gap-1">
                        <div
                            class="flex min-w-40 flex-1 flex-row gap-1 rounded-xl p-2 lg:gap-3 lg:p-3"
                            v-for="(order, j) in showItem.orders"
                            :class="{
                                'bg-surface-200': j % 2 === 1,
                                'bg-surface-100': j % 2 === 0,
                                'outline-primary rounded-xl outline-2': order.selected,
    ...
    <Popover ref="alertsPopover">
        <div class="flex flex-col gap-1">
            <div
                v-for="(alert, j) in showAlerts"
                :class="{
                    'bg-surface-200': j % 2 === 1,
                    'bg-surface-100': j % 2 === 0,
                }"
                class="flex flex-col gap-1 rounded-xl p-3"
            >
                <span>{{ alert.AAName }}</span>
                <span>Zgłoszono: {{ formatDate(alert.date, 'dd.MM.yyyy HH:mm') }}</span>
                <span
                    >Stan: <Tag :severity="getSeverity(alert.State)" :value="getAlertMessage(alert.State)"></Tag>
                </span>
            </div>
        </div>
    </Popover>
...
<script setup>
    ...
    function runAfter10MinOfInactivity(callback) {
        let timeoutId;
        const events = ['click', 'keydown', 'mousemove', 'scroll', 'touchstart', 'wheel', 'pointerdown'];
    ...
    function declareSocket() {
        if (socket) socket.disconnect();
        socket = connect();
    }

    function declareSocketActions(clear = false) {
        if (clear) {
            declareSocket();
        }
        socket.on('updateDisplay', async data => {
            displayUUID.value = localStorage.getItem('displayUUID');
            try {
                await api.post('/displays/login', {
                    UUID: displayUUID.value,
        ...
        socket.on('updateElement', data => {
            ...
            if (element.ETID == 1) {
                ...
                Object.assign(
                    element.data.mapData.find(el => el.EID == data.EID),
                    data
                );
                ...
                items.value.filter(async item => {
                    ...
                    if (!orderParent) {
                        ...
                        if (order) {
                            ...
                            try {
                                data.orderElement.alerts = (
                                    await api.get(`/alerts/by-eid/${data.orderElement.EID}`)
                                ).data.map(alert => {
                                    return {
                                        ...alert,
                                        AAName: allAlertTypes.find(at => at.AAID == alert.AAID).name,
                                ...
                                toast.add(
                                    toastHandler('error', 'Wystąpił problem', 'Nie udało się pobrać alertów.', error)
                                );
                            }

                            data.orderElement.newAlertPresent = data.orderElement.alerts.some(
                                alert => alert.State == 0
                            );
        ...
        socket.on('updateSectorUpdateOrder', async data => {
            if (data.oldParentEID == data.orderElement.ParentEID) {
                Object.assign(
                    items.value
                        .find(item => item.EID == data.orderElement.ParentEID)
                        .orders.find(el => el.EID == data.orderElement.EID),
                    data.orderElement
                );
            } else {
                const oldParentItem = items.value.find(
                    item => item.EID == data.oldParentEID && data.roomEID == item.EID
                );
                if (oldParentItem) {
                ...
                const newParentItem = items.value.find(
                    item => item.EID == data.orderElement.ParentEID && data.roomEID == item.EID
                );
                if (newParentItem) {
                    data.orderElement.alerts = [];
                    try {
                        data.orderElement.alerts = (await api.get(`/alerts/by-eid/${data.orderElement.EID}`)).data.map(
                            alert => {
                                return {
                                    ...alert,
                                    AAName: allAlertTypes.find(at => at.AAID == alert.AAID).name,
                        ...
                        toast.add(toastHandler('error', 'Wystąpił problem', 'Nie udało się pobrać alertów.', error));
                    }
        ...
        socket.on('newAlert', data => {
            let roomEID = data.roomEID;
            delete data.roomEID;
            data.date = new Date();
            let item = items.value.find(item => item.EID == data.EID && roomEID == item.EID);

            if (item) {
                if (!item.data.alerts.some(alert => alert.AID == data.AID)) {
                    item.data.alerts.push(data);
                    item.data.newAlertPresent = true;
                }
            } else {
                items.value.forEach(item => {
                    if (item.EID != roomEID) return;
                    ...
                    if (item.ETID == 3) {
                        if (item?.orders?.length > 0) {
                            ...
                            if (!order.alerts.some(alert => alert.AID == data.AID)) {
                        ...
                        if (item?.data?.mapData?.length > 0) {
                            ...
                            if (!element.alerts.some(alert => alert.AID == data.AID)) {
                                element.alerts.push(data);
                            }
                            if (element.ETID == 2) {
                                let sector = item.data.mapData.find(el => el.EID == element.ParentEID);
                                if (!sector.alerts.some(alert => alert.AID == data.AID)) {
                                    sector.alerts.push(data);
                                }
                            }
                        }
                    }
        ...
        socket.on('updateAlert', data => {
            ...
            if (item) {
                ...
                if (item.data.alerts.length == 0) item.data.newAlertPresent = false;
                    ...
                    item.data.alerts.find(el => el.State == 0)
                        ? (item.data.newAlertPresent = true)
                        : (item.data.newAlertPresent = false);
            } else {
                items.value.forEach(item => {
                    if (item.EID != roomEID) return;
                    ...
                    if (item.ETID == 3) {
                        if (item?.orders.length > 0) {
                            ...
                            if (order.alerts.length == 0) order.newAlertPresent = false;
                                ...
                                order.alerts.find(el => el.State == 0)
                                    ? (order.newAlertPresent = true)
                                    : (order.newAlertPresent = false);
                        }
                    } else if (item.ETID == 1) {
                        if (item?.data?.mapData?.length > 0) {
                            let element = item.data.mapData.find(el => el.EID == data.EID);
                            ...
                            if (element.ETID == 2) {
                                let sector = item.data.mapData.find(el => el.EID == element.ParentEID);
                                if (data.State == 2) sector.alerts = sector.alerts.filter(el => el.AID != data.AID);
                                else sector.alerts.find(el => el.AID == data.AID).State = data.State;
                            }
                        }
                    }
                });
                ...
                items.value.forEach(item => {
                    ...
                    if (item.ETID == 3) {
                        ...
                        if (item?.data?.mapData?.length > 0) {
                            ...
                            if (element.ETID == 2) {
                                let sector = item.data.mapData.find(el => el.EID == element.ParentEID);
                                sector.alerts = sector.alerts.filter(el => el.AID != data.AID);
                            }
                        }
                    }
                });
