        ...
        <Card :pt="{ body: 'p-2 lg:p-5' }">
            <template #content>
                <DataTable
                    :items="items"
                    :columns="columns"
                    :advancedInteractionsAvailable="true"
...
<script setup>
    ...
    function editItem(item) {
        if (!item) {
            toast.add(
                toastHandler('warn', 'Nie wybrano użytkownika', 'Wybierz użytkownika którego chcesz zmodyfikować')
            );
            return;
        }
