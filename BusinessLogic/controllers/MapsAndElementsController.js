...
async function allowenceCheck(allowedMaps, EIDs) {
    ...
    return rawData.map(row => MapsAndElements.build(row, { isNewRecord: false }));
}
...
async function getMaps(req, res) {
    try {
        ...
        return res.status(500).json({ error: 'Nie udało się pobrać map', details: error.message });
    }
}
...
async function getSectors(req, res) {
    try {
        ...
        return res.status(500).json({ error: 'Nie udało się pobrać sektorów', details: error.message });
    }
}

async function getSingleObject(req, res) {
    try {
        let allowedMaps = getAllowedMaps(req.cookies['WarehouseLogisticsToken']);
        ...
        return res.status(500).json({ error: 'Nie udało się pobrać obiektu', details: error.message });
    }
}

async function getMapWithChildren(req, res) {
    try {
        let allowedMaps = getAllowedMaps(req.cookies['WarehouseLogisticsToken']);
        const query = `
          WITH RECURSIVE MapElements AS (
        ...
        return res.status(500).json({ error: 'Nie udało się pobrać elementów', details: error.message });
    }
}

async function createMapElement(req, res) {
    let transaction;
    try {
        ...
        return res.status(500).json({ error: 'Nie udało się utworzyć elementu', details: error.message });
    }
}

async function updateMapElement(req, res) {
    try {
        ...
        return res.status(500).json({ error: 'Nie udało się zaktualizować elementu', details: error.message });
    }
}

async function deleteMapElement(req, res) {
    try {
        let allowedMaps = getAllowedMaps(req.cookies['WarehouseLogisticsToken']);
        let EIDs = [req.params.id];
        const allowence = await allowenceCheck(allowedMaps, EIDs);
        ...
        return res.status(500).json({ error: 'Nie udało się usunąć elementu', details: error.message });
    }
}
