...
async function createOrder(req, res) {
    ...
    try {
        ...
        return res.status(500).json({ error: `Nie udało się utworzyć zlecenia`, details: error.message });
    }
}

async function getOrdersByEID(req, res) {
    try {
        let allowedMaps = getAllowedMaps(req.cookies['WarehouseLogisticsToken']);
        ...
        return res.status(404).json({ error: 'Nie znaleziono zlecenia', details: error.message });
    }
}
...
async function getOrderById(req, res) {
    try {
        ...
        return res.status(500).json({ error: 'Nie udało się pobrać zlecenia', details: error.message });
    }
}
...
async function updateOrder(req, res) {
    ...
    try {
        ...
        if (transaction) await transaction.rollback();
        return res.status(500).json({ error: 'Nie udało się zaktualizować zlecenia', details: error.message });
    }
}
...
async function deleteOrder(req, res) {
    ...
    try {
        ...
        if (transaction) await transaction.rollback();
        return res.status(500).json({ error: 'Nie udało się usunąć zlecenia', details: error.message });
    }
}
